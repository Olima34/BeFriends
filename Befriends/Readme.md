...
│   ├── BluetoothManager.swift   # Logique BLE centrale, sessions, sync, persistance
...
### 1. Découverte
Le `CBCentralManager` scanne en permanence les périphériques exposant le service Befriends. A la découverte :
...
- Le timer `checkExpirations` tourne toutes les **30 secondes** (via `DispatchSourceTimer` pour fonctionner en arrière-plan)
...
- **Retour au premier plan** (`scenePhase == .active`) → `checkExpirations()` immédiat
...
Les timers utilisent `DispatchSourceTimer` (GCD) au lieu de `Timer` (RunLoop) car les timers RunLoop s'arrêtent quand l'app est suspendue.
