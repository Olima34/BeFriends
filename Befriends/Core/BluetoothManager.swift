...
class BluetoothManager: NSObject, ObservableObject {
        ...
        for central in subscribedCentrals {
            ...
            if let friendID = centralToFriendID[central.identifier],
               let session = activeSessions.first(where: { $0.id == friendID }) {
                ...
                // Fallback si le mapping n'existe pas encore
                duration = activeSessions.max(by: { $0.duration < $1.duration })?.duration ?? 0
            }
    ...
    func acceptFriend(_ session: ActiveSession) {
        ...
        if !activeSessions.contains(where: { $0.id == session.id }) {
            ...
            // pour éviter une session fantôme si l'appareil est déjà hors de portée
            let isStillConnected = session.peripheral?.state == .connected
            var newSession = session
            newSession.isConnected = isStillConnected
