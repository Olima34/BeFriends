...
let BLE_CLEANUP_INTERVAL: TimeInterval = 60.0 // Fréquence de nettoyage des timers BLE
let BLE_BROADCAST_INTERVAL: TimeInterval = 30.0 // Fréquence de diffusion du temps (GATT)
...
let REDISCOVERY_DEBOUNCE: TimeInterval = 30.0 // Ignorer les re-découvertes BLE trop rapides
let DEDUP_COOLDOWN: TimeInterval = 120.0 // Cooldown entre les vérifications de doublons Firebase par ami

// 2. SENSIBILITÉ DE LA SYNCHRONISATION (HANDSHAKE)
let SYNC_THRESHOLD: TimeInterval = 30.0 // Écart toléré avant de se synchroniser sur l'autre
let FORCE_PUSH_THRESHOLD: TimeInterval = 60.0 // Écart pour forcer l'autre à s'aligner
