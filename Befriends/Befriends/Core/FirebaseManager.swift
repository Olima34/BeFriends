...
class FirebaseManager: ObservableObject {
    ...
    /// Génère un ID de document déterministe pour éviter les doublons de sessions
    func deterministicSessionID(participants: [String], date: Date) -> String {
        let sorted = participants.sorted().joined(separator: "-")
        let roundedTimestamp = Int(date.timeIntervalSince1970 / 60) * 60  // Arrondi à la minute
        return "\(sorted)-\(roundedTimestamp)"
    }
    ...
    // 👇 SUPPRIMEZ TOTALEMENT LA FONCTION "func signInAnonymously() { ... }" DE VOTRE FICHIER.
    ...
    func saveHistoryItem(_ item: HistorySession) {
        ...
        // ID déterministe : les deux téléphones écrivent le même document → pas de doublon
        let docID = deterministicSessionID(participants: [myStableID, item.friendID], date: item.date)
        db.collection("shared_history").document(docID).setData(data, merge: true) { error in
            if let error = error {
    ...
    func listenToHistory(completion: @escaping ([HistorySession]) -> Void) -> ListenerRegistration? {
        ...
        // On cherche les sessions récentes où NOTRE ID est dans les participants (max 100)
    ...
    func listenForCommands(myStableID: String, onCommandReceived: @escaping (String, String) -> Void) {
        db.collection("mailboxes").document(myStableID).collection("commands").addSnapshotListener { [weak self] snapshot, _ in
            guard let self = self, let docs = snapshot?.documents else { return }
            for doc in docs {
                guard !self.processedCommandIDs.contains(doc.documentID) else { continue }
                self.processedCommandIDs.insert(doc.documentID)
