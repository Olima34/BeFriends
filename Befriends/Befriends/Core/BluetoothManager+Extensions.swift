    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, didReceiveRead request: CBATTRequest) {
        ...
        if let friendID = centralToFriendID[request.central.identifier] {
            if let session = activeSessions.first(where: { $0.id == friendID && $0.isConnected }) {
                ...
                duration = waitingForMutual.contains(friendID) ? 0 : -1
            } else {
                duration = -1
            }
        } else {
                ...
                if let session = activeSessions.first(where: { $0.peripheral?.identifier == peripheral.identifier }) {
                    ...
                    if remoteDuration == -1 {
                        ...
                        if remoteDuration == 0 && session.duration > FRESH_SESSION_THRESHOLD {
                            ...
                            if let idx = activeSessions.firstIndex(where: { $0.id == session.id }) {
                                activeSessions[idx].startTime = Date()
                                // Pas de updateBroadcastedTime() ici — le cycle régulier (5s)
                                // enverra la bonne durée à chaque central individuellement,
                                // sans écraser les sessions des autres amis.
                            }
                        }
                    }
