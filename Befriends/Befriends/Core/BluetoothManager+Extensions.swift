...
extension BluetoothManager: CBCentralManagerDelegate, CBPeripheralDelegate, CBPeripheralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        // Debounce de redécouverte : ignorer les advertisements répétés du même peripheral
        if let last = lastDiscoveredTime[peripheral.identifier],
           Date().timeIntervalSince(last) < REDISCOVERY_DEBOUNCE {
            return
        }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        for characteristic in characteristics {
            ...
            if characteristic.uuid == TIME_SYNC_CHARACTERISTIC_UUID {
                // FIX #1 : Envoyer l'identification AVANT de s'abonner.
                // Avec .withResponse, le GATT sérialise : le write est acquitté avant
                // que le subscribe soit traité, donc le mapping est en place quand
                // didSubscribeTo se déclenche côté périphérique.
        ...
        if characteristic.uuid == TIME_SYNC_CHARACTERISTIC_UUID {
            if let timeString = String(data: data, encoding: .utf8), let remoteDuration = TimeInterval(timeString) {
                if let session = activeSessions.first(where: { $0.peripheral?.identifier == peripheral.identifier }) {
                    ...
                    // FIX #4 : Pas de DispatchQueue.main.async — les callbacks CoreBluetooth
                    // (queue: nil) sont déjà sur le main thread. L'async créait des fenêtres
                    // de désynchronisation permettant un double traitement de -1.
                    if remoteDuration == -1 {
                        // FIX #1 : Compteur de -1 consécutifs. Ne déclencher la suppression
                        // qu'après 3 confirmations (~15s) pour absorber un -1 résiduel.
                        let count = (consecutiveNegativeOnes[session.id] ?? 0) + 1
                        consecutiveNegativeOnes[session.id] = count

                        if !waitingForMutual.contains(session.id) {
                            if session.duration > INITIAL_NEGATIVE_GRACE_PERIOD && count >= REMOTE_DELETION_CONFIRMATION_COUNT {
                                processRemoteDeletion(friendID: session.id)
                                return
                            } else if session.duration <= INITIAL_NEGATIVE_GRACE_PERIOD {
                                waitingForMutual.insert(session.id)
                            }
                        }

                        if let idx = activeSessions.firstIndex(where: { $0.id == session.id }) {
                            activeSessions[idx].startTime = Date()
                        }
                        return
                    } else {
                        ...
                        if waitingForMutual.contains(session.id) {
                            // FIX #2 : Seule une durée FAIBLE indique une nouvelle session fraîche
                            if remoteDuration < FRESH_SESSION_THRESHOLD {
                                let positiveCount = (consecutivePositiveOnes[session.id] ?? 0) + 1
                                consecutivePositiveOnes[session.id] = positiveCount
                                
                                if positiveCount >= MUTUAL_ACCEPT_CONFIRMATION_COUNT {
                                    // Confirmation
                                    waitingForMutual.remove(session.id)
                                    consecutivePositiveOnes.removeValue(forKey: session.id)
                                    
                                    if let idx = activeSessions.firstIndex(where: { $0.id == session.id }) {
                                        activeSessions[idx].isConnected = true
                                        activeSessions[idx].startTime = Date().addingTimeInterval(-remoteDuration)
                                    }
                                }
                            } else {
                                // Durée élevée = ancienne session périmée, on ignore et on reset le compteur
                                consecutivePositiveOnes.removeValue(forKey: session.id)
                            }
                        }
                        if waitingForMutual.contains(session.id) {
                            // ... (votre code existant qui gère le FRESH_SESSION_THRESHOLD)
                        }
                        
                        // 👇 FIX 3 : AJOUTEZ CE BLOC ICI
                        // L'autre nous envoie "0" (signal neutre d'attente) mais nous avons une vieille session active.
                        // Cela signifie qu'il vient de nous re-accepter. On réinitialise notre session pour l'aider !
                        if remoteDuration == 0 && session.duration > FRESH_SESSION_THRESHOLD {
                            print("⚠️ L'ami est en attente (envoie 0). On réinitialise notre session pour le débloquer !")
                            if let idx = activeSessions.firstIndex(where: { $0.id == session.id }) {
                                activeSessions[idx].startTime = Date()
                                self.updateBroadcastedTime()
                            }
                        }
                    }
                    ...
                    if session.isConnected && remoteDuration > (session.duration + SYNC_THRESHOLD) {
                        // Debounce : ne vérifier les doublons qu'une fois par DEDUP_COOLDOWN par ami
                        if lastDedupTime[session.id] == nil || Date().timeIntervalSince(lastDedupTime[session.id]!) >= DEDUP_COOLDOWN {
                            lastDedupTime[session.id] = Date()
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, didReceiveWrite requests: [CBATTRequest]) {
                       ...
                       let newTime = TimeInterval(components[1]) {
                        ...
                        if let index = self.activeSessions.firstIndex(where: { $0.id == senderID }) {
                            if !self.activeSessions[index].isConnected {
                                if !self.waitingForMutual.contains(senderID) {
                            ...
                            if session.isConnected && newTime > (session.duration + DUPLICATE_SEARCH_WINDOW) {
                                ...
                                // Debounce : ne vérifier les doublons qu'une fois par DEDUP_COOLDOWN par ami
                                if self.lastDedupTime[senderID] == nil || Date().timeIntervalSince(self.lastDedupTime[senderID]!) >= DEDUP_COOLDOWN {
                                    self.lastDedupTime[senderID] = Date()
        ...
        // FIX #6 : Répondre une seule fois pour le batch entier.
        // La doc Apple exige un seul respond par invocation de didReceiveWrite,
        // en passant le premier request du tableau.
