...
class BluetoothManager: NSObject, ObservableObject {
    ...
    func updateBroadcastedTime() {
        ...
        guard let char = timeSyncCharacteristic,
              let maxSession = activeSessions.max(by: { $0.duration < $1.duration }) else { return }
        
        let durationString = "\(Int(maxSession.duration))"
        if let data = durationString.data(using: .utf8) {
    ...
    func handleIdentification(stableID: String, name: String, peripheral: CBPeripheral) {
        DispatchQueue.main.async {
            let now = Date()
            // 🛑 NOUVEAU : NETTOYAGE PRÉVENTIF AU RÉVEIL
            // Si le Timer était endormi, on fait son travail ici avant de reconnecter !
            if let index = self.activeSessions.firstIndex(where: { $0.id == stableID }) {
            ...
            if self.savedFriends.contains(where: { $0.id == stableID }) {
                self.sendProximityNotification(forName: name, id: stableID)
                if let index = self.activeSessions.firstIndex(where: { $0.id == stableID }) {
                    self.activeSessions[index].isConnected = true
                    self.activeSessions[index].lastSeenTime = now
                    self.activeSessions[index].peripheral = peripheral
                } else {
                ...
                if !self.pendingRequests.contains(where: { $0.id == stableID }) {
                    ...
                    self.sendNewDeviceNotification(forName: name, id: stableID)
                }
                self.centralManager.cancelPeripheralConnection(peripheral)
                self.connectionBlacklist[peripheral.identifier] = now
            }
        }
    }
    ...
    func saveData() {
        for i in 0..<activeSessions.count {
            if activeSessions[i].isConnected { activeSessions[i].lastSeenTime = Date() }
        }
