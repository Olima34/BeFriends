...
class BluetoothManager: NSObject, ObservableObject {
    ...
    func acceptFriend(_ session: ActiveSession) {
        ...
        if let index = activeSessions.firstIndex(where: { $0.id == session.id }) {
            ...
            // 🌟 NOUVEAU : On ne recycle pas l'ancienne date de la demande en attente !
            // On force un startTime à Date() (maintenant) pour commencer à 0.
            let newSession = ActiveSession(
                id: session.id,
    ...
    func checkExpirations() {
        ...
        // Annuler les connexions pendantes périmées (rotation d'adresse BLE).
        // Après 2 min sans nouvelles, le peripheral est probablement hors de portée
        // ou a changé d'adresse. Annuler libère des ressources CoreBluetooth
        // et permet au scan de découvrir le nouveau peripheral.
    ...
    func removeFriend(id: String) {
        ...
        if let index = activeSessions.firstIndex(where: { $0.id == id }) {
            ...
            if !pendingRequests.contains(where: { $0.id == id }) {
                ...
                pendingSession.isConnected = true
