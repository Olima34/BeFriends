...
class BluetoothManager: NSObject, ObservableObject {
    ...
    /// Indique si les services BLE sont actifs (authentifié et prêt)
    private(set) var isServiceActive = false
    ...
    override init() {
        super.init()
        loadData()
        // Création des managers BLE (nécessaire pour la restauration d'état en background).
        // Le scan et l'advertising ne démarrent que quand startServices() est appelé.
    ...
    /// Démarre tous les services BLE, timers et listeners Firebase.
    /// Appelé après l'authentification ou au lancement si déjà connecté.
    func startServices() {
        guard Auth.auth().currentUser != nil else { return }
        guard !isServiceActive else { return }
        isServiceActive = true

        // Charger les amis depuis Firebase (source de vérité multi-appareils)
        loadFriendsFromFirebase()

        // Timers (DispatchSource : fonctionnent en arrière-plan actif)
        ...
        // Démarrer BLE si les managers sont déjà prêts
    ...
    /// Charge les amis depuis Firebase et réconcilie avec les pending requests.
    private func loadFriendsFromFirebase() {
        FirebaseManager.shared.loadFriends { [weak self] friends in
            DispatchQueue.main.async {
                guard let self = self else { return }
                self.savedFriends = friends
                self.saveData()

                // Réconciliation : les pending requests qui sont des amis connus
                // doivent devenir des sessions actives
                let recognized = self.pendingRequests.filter { pending in
                    friends.contains(where: { $0.id == pending.id })
                }
                for pending in recognized {
                    self.pendingRequests.removeAll { $0.id == pending.id }
                    if !self.activeSessions.contains(where: { $0.id == pending.id }) {
                        let session = ActiveSession(
                            ...
                            isConnected: true,
                            lastSeenTime: Date(),
                            peripheral: pending.peripheral
                        )
                        self.activeSessions.append(session)
                    }
                    if let peripheral = pending.peripheral {
                        self.connectionBlacklist.removeValue(forKey: peripheral.identifier)
    ...
    func updateBroadcastedTime() {
        ...
        for central in subscribedCentrals {
            ...
            if let session = activeSessions.first(where: { $0.id == friendID }) {
                if session.isConnected && !waitingForMutual.contains(friendID) {
                    // Session active, ami accepté mutuellement → vraie durée
                    if let data = "\(Int(session.duration))".data(using: .utf8) {
                        peripheralManager.updateValue(data, for: char, onSubscribedCentrals: [central])
                    }
                } else if waitingForMutual.contains(friendID) {
                    // En attente d'acceptation mutuelle → 0
                    if let data = "0".data(using: .utf8) {
                        peripheralManager.updateValue(data, for: char, onSubscribedCentrals: [central])
                    }
                }
                // Session en grace period (isConnected = false), pas waitingForMutual → ne rien envoyer
    ...
    func acceptFriend(_ session: ActiveSession) {
        ...
        consecutiveNegativeOnes.removeValue(forKey: session.id)
        consecutivePositiveOnes.removeValue(forKey: session.id)
    ...
    func checkExpirations() {
        ...
        for index in activeSessions.indices.reversed() {
            ...
            if !session.isConnected {
                // Ne pas archiver une session en attente d'acceptation mutuelle
                if waitingForMutual.contains(session.id) {
                    continue
                }
        ...
        // Nettoyage des waitingForMutual orphelins (plus de session active ni de savedFriend)
        let orphanedWaiting = waitingForMutual.filter { id in
            !activeSessions.contains(where: { $0.id == id }) &&
            !savedFriends.contains(where: { $0.id == id })
        }
        for id in orphanedWaiting {
    ...
    func handleIdentification(stableID: String, name: String, peripheral: CBPeripheral) {
        ...
        if savedFriends.contains(where: { $0.id == stableID }) {
            ...
            if let index = activeSessions.firstIndex(where: { $0.id == stableID }) {
                ...
                // Reset des compteurs à chaque reconnexion BLE
                consecutiveNegativeOnes.removeValue(forKey: stableID)
                // Ne pas reset consecutivePositiveOnes si on est en waitingForMutual
                // (sinon on perd le progrès de validation)
                if !waitingForMutual.contains(stableID) {
                    consecutivePositiveOnes.removeValue(forKey: stableID)
                }
    ...
    func restartScanning() {
        guard isServiceActive, centralManager.state == .poweredOn else { return }
    ...
    func removeFriend(id: String) {
        consecutiveNegativeOnes.removeValue(forKey: id)
        consecutivePositiveOnes.removeValue(forKey: id)
        ...
        // 3. On nettoie tout (local + Firebase)
        savedFriends.removeAll { $0.id == id }
        waitingForMutual.remove(id)
        FirebaseManager.shared.deleteFriend(id)
        saveData()
    ...
    func processRemoteDeletion(friendID: String) {
         ...
         // 2. Supprimer l'ami de notre téléphone (local + Firebase)
         savedFriends.removeAll { $0.id == friendID }
         waitingForMutual.remove(friendID)
         FirebaseManager.shared.deleteFriend(friendID)
         saveData()
         restartScanning()
     }
    
    /// Arrête tous les services BLE, timers, listeners, et nettoie les données locales.
    /// Appelé à la déconnexion du compte.
    func stopServices() {
        print("🧹 Arrêt complet des services suite à la déconnexion...")
        isServiceActive = false

        // 1. Stopper les timers
        cleanupTimerSource?.cancel()
        cleanupTimerSource = nil
        broadcastTimerSource?.cancel()
        broadcastTimerSource = nil
        saveWorkItem?.cancel()
        saveWorkItem = nil

        // 2. Stopper le listener Firebase
        FirebaseManager.shared.stopListeningForCommands()

        // 3. Déconnecter tous les peripherals BLE actifs
        ...
        // 4. Vider la mémoire vive
        savedFriends.removeAll()
        activeSessions.removeAll()
        pendingRequests.removeAll()
        waitingForMutual.removeAll()
        discoveredPeripherals.removeAll()
        subscribedCentrals.removeAll()
        centralToFriendID.removeAll()
        consecutiveNegativeOnes.removeAll()
        consecutivePositiveOnes.removeAll()
        connectionBlacklist.removeAll()
        notificationCooldowns.removeAll()

        // 5. Vider le disque (UserDefaults)
