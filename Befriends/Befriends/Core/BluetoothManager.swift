...
class BluetoothManager: NSObject, ObservableObject {
    ...
    func updateBroadcastedTime() {
        ...
        // Nettoyer les centrals qui n'ont pas de mapping du tout (déconnexion brutale)
        // MAIS garder ceux qui ont un mapping même sans session (= pas encore accepté)
        ...
        for central in subscribedCentrals {
            ...
            if let session = activeSessions.first(where: { $0.id == friendID && $0.isConnected }) {
                // Ami accepté, session active → envoyer la vraie durée
                ...
                // Ami dans notre liste mais pas de session active connectée
                // (ex: session en grace period, ou tout juste accepté) → ne rien envoyer
            } else {
                // Pas dans nos amis → envoyer -1 pour signaler "pas accepté"
