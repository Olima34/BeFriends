...
struct FriendProfileView: View {
                    ...
                    HStack {
                        ...
                        if let session = activeSession {
                            if manager.waitingForMutual.contains(friend.id) {
                                Text("En attente d'acceptation").foregroundColor(.yellow)
                            } else if session.isConnected {
                                Text("Connecté").foregroundColor(.green)
                            } else {
                                Text("Signal perdu").foregroundColor(.orange)
                            }
                            
                            // N'affichez la durée QUE si on n'est plus en attente mutuelle
                            if !manager.waitingForMutual.contains(friend.id) {
    ...
    func formatLiveDuration(_ duration: TimeInterval) -> String {
    ...
    // FIX #9 : DateFormatter statiques — évite de recréer des instances coûteuses
    // à chaque session dans le ForEach (100+ instanciations par re-render).
    private static let timeFormatter: DateFormatter = {
        let f = DateFormatter(); f.timeStyle = .short; return f
    }()
    private static let dateFormatter: DateFormatter = {
        let f = DateFormatter(); f.dateStyle = .medium; f.timeStyle = .none
        f.doesRelativeDateFormatting = true; f.locale = Locale(identifier: "fr_FR")
        return f
    }()
