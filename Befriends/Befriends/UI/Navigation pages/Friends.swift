...
struct FriendsView: View {
    // FIX #7 : Reçu depuis ContentView (plus de @StateObject locaux)
    @ObservedObject var manager: BluetoothManager
    @ObservedObject var historyVM: HistoryViewModel

    // FIX #10 : Timer géré manuellement pour ne tourner que quand l'onglet est visible
    ...
    var body: some View {
        NavigationStack {
            ...
            // FIX #10 : Le timer ne tourne que quand la vue est visible
            .onAppear {
...
struct FriendStatusRow: View {
    ...
    var body: some View {
        HStack(spacing: 15) {
            ...
            if let session = activeSession, !manager.waitingForMutual.contains(friend.id) {
